...
class Moonshine::Manifest::RailsNginx < Moonshine::Manifest
  def validate_platform
    unless Facter.lsbdistid == 'Ubuntu' && Facter.lsbdistrelease.to_f >= 8.04
      error = <<-ERROR


      Moonshine::Manifest::RailsNginx is currently only supported on Ubuntu 8.04
  ...
  require File.join(File.dirname(__FILE__), 'rails', 'nginx.rb')
  include Moonshine::Manifest::Rails::Passenger
